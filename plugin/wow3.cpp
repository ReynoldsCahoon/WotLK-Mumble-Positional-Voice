...
static int fetch(float *avatar_pos, float *avatar_front, float *avatar_top, float *camera_pos, float *camera_front,
    ...
    procptr_t leaderguid_address        = 0x00BD1968;   // OK
    ...
    ok = peekProc(state_address, &state, 1) &&                             // Magical state value: 1 when in-game, 0 when not.
        ...
        peekProc(leaderguid_address, leaderGUID);                          // Leader GUID.
    ...
    // Map ID
    if (mapId >= 0)
        ocontext << std::endl << "\"map\": " << mapId; // Set context with mapid
    else
        ocontext << std::endl << "\"map\":  None"; // Set context with mapid
    ...
    if (strcmp(player, "") != 0)
        oidentity << std::endl << "\"char\": \"" << player << "\","; // Set player nickname in identity.
    else 
        oidentity << std::endl << "\"char\": None,";

    // Leader GUID
    if (leaderGUID > 0)
        oidentity << std::endl << "\"leaderguid\": " << leaderGUID; // Set context with leaders guid
    else
        oidentity << std::endl << "\"leaderguid\":  0"; // Set guid to 0 if on a continent map
...
static const std::wstring longdesc() {
    return std::wstring(L"Supports World of Warcraft (x86) with identity support.");    // Plugin long description
}
